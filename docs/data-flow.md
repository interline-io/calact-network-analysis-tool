...
| 2 | `stops` | `fetchStops(task)` via `stopFetchQueue` | up to 8 in flight | Per feed version, paginates `stops(limit, after, where: {feed_version_sha1, location: {bbox|geography_ids}})`. Each page batches `PROGRESS_LIMIT_STOPS = 1000` stops per progress event. Each stop carries nested `route_stops` so we discover route IDs without an extra hop. Census geographies used to ride along here too; they are now a separate `stop-census` pass, so route IDs are not held up by them. |
